...
public class AM1_NcoWebClientSanity extends Base {
...
//	@Test(priority=4)
	public void AM1_Netsfere_Cleanup() throws InterruptedException, IOException {
		...
		NetsfereActivity.killAllBrowserSessions();
	    System.out.println("AM1 Cleanup has been completed: Success*******.");
	}
...
//				System.out.println("Used Storage in MB :"+AM1Vault_OldData_map.get("UsedStorage"));
		...
		System.out.println("AM1 REGION - Basic health checks started..."); 
		...
		if( Config.getInstance().isDebug()) {
			webUserId = Config.getInstance().getAM1WebIdTest();
			webUserDisplayName = Config.getInstance().getAM1WebDisplayNameTest();
			chromeUserId = Config.getInstance().getAM1ChromeIdTest();
			chromeUserDisplayName = Config.getInstance().getAM1ChromeDisplayNameTest();
			firefoxUserId =  Config.getInstance().getAM1FirefoxIdTest();
			firefoxUserDisplayName = Config.getInstance().getAM1FirefoxDisplayNameTest();
			
		} else {
			webUserId = Config.getInstance().getAM1WebId();
			webUserDisplayName = Config.getInstance().getAM1WebDisplayName();
			chromeUserId = Config.getInstance().getAM1ChromeId();
			chromeUserDisplayName = Config.getInstance().getAM1ChromeDisplayName();
			firefoxUserId =  Config.getInstance().getAM1FirefoxId();
			firefoxUserDisplayName = Config.getInstance().getAM1FirefoxDisplayName();
		}
...
//		int rc = NetsfereActivity.webClientLogin(webDriver, Config.getInstance().getAM1WebId());	
		...
		if( rc != 0 ) {
			...
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to AM1 Web");
	        System.exit(0);
		}	
...
//		rc = NetsfereActivity.webClientLogin(foxDriver, Config.getInstance().getAM1FirefoxId());		
		rc = NetsfereActivity.webClientLogin(foxDriver,firefoxUserId);
		if( rc != 0 ) {
			System.out.println("ERROR -netsfere Login page Not Loaded. Pls Check Internet Connection...");	        
			System.out.println("exiting the programe...");
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to AM1 Firefox");
	        System.exit(0);
		}	
		...
		System.out.println("ChromAM1ser Starting Conversation With Firefoxuser.");
...
//			webDriver.findElementByXPath("//input[@item='inviteContacts']").sendKeys(Config.getInstance().getAM1FirefoxDisplayName());	
...
//			webWait20.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='mainDiv']//div[2]//div[@class='scrollbox']/div[@class='table-view']//div[4]/div[@displayname='"+Config.getInstance().getAM1FirefoxDisplayName()+"']")));			
//			webDriver.findElementByXPath("//div[@class='mainDiv']//div[2]//div[@class='scrollbox']/div[@class='table-view']//div[4]/div[@displayname='"+Config.getInstance().getAM1FirefoxDisplayName()+"']").click();
//			Thread.sleep(1000);
			...
			try {
				...
				Assert.assertFalse(true,"Error AM1 WebUser Unable to Create COnversation with AM1 Firefox User");
			}
			...
			if(webDriver.findElementByXPath("//textarea[@class='namegenTextLong']") != null) {
				...
				Message1 = Message1 +" : AM1 - Hello from Webuser";
				...
				if (sentStaus.getAttribute("title").contains("Read") || sentStaus.getAttribute("title").contains("Sent") ) {
					...
					Assert.assertFalse(true,"Error AM1 WebUser Unable to send message");
				}
			}
		} catch(Exception e) {
			System.out.println("Webuser Unable to create conversation with Firefoxuser.");
		...
		try {
			...
			System.out.println("ERROR- Conversation created by Webuser with Firefoxuser does not exists on Firefoxuser side Or Firefoxuser did not receive the message sent by ChromAM1ser.");
			Assert.assertFalse(true,"ERROR - AM1 Firefox user did not get the message");
		}
...
//				NetsfereActivity.attachImage_win();
		...
		try {
			...
			if(elems4.size() > 0) {
				...
				if ( el1 != null ) {					
					System.out.println("ChromAM1ser Received Image");
				}
			}
		} catch ( NoSuchElementException e) {		
			System.out.println("ChromAM1ser DID NOT recieve the IMAGE from ChromAM1ser....  ");
		...
		// Webuser make a call to webAM1ser2..
...
//		webDriver.findElementByXPath("//span[@class='material-icons' and contains(text(),'search')]/../../..//input").sendKeys(Config.getInstance().getAM1FirefoxDisplayName());
		...
		try {
			...
			if ( foxDriver.findElementByXPath("//span[contains(text(),'Incoming call...')]") != null ) {				
				System.out.println("Firefoxuser Received the HD call from ChromAM1ser");
			}
		...
		System.out.println("Login to Second ChromAM1ser...");
		...
		if( rc != 0 ) {
			...
			System.out.println("exiting the programe...");
			webDriver.close();
			Assert.assertFalse(true,"Unable to login to ChromAM1ser");
		    System.exit(0);
		}		
		...
		try {
			...
			if ( foxDriver.findElementByXPath("//div[contains(text(),'Incoming call...')]") != null ) {	
				System.out.println("Firefoxuser Received the group call from SecondChromAM1ser");
			}
