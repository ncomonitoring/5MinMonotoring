...
public class UKF_NcoWebClientSanity extends Base {
	...
	//	@Test(priority=4)
	public void UKF_Netsfere_Cleanup() throws InterruptedException, IOException {
		...
		NetsfereActivity.killAllBrowserSessions();
		System.out.println("UKF  Cleanup has been completed: Success*******.");
	}

	@Test(priority=5)
	public void UKF_Netsfere_Message_Call_Check() throws InterruptedException, IOException , AWTException {		

		NetsfereActivity.killAllBrowserSessions();	

		System.out.println("=================================================================");
		System.out.println("******** UKF REGION - Basic health checks started... ************"); 
		...
		if( Config.getInstance().isDebug()) {
			webUserId = Config.getInstance().getUKFWebIdTest();
			webUserDisplayName = Config.getInstance().getUKFWebDisplayNameTest();
			chromeUserId = Config.getInstance().getUKFChromeIdTest();
			chromeUserDisplayName = Config.getInstance().getUKFChromeDisplayNameTest();
			firefoxUserId =  Config.getInstance().getUKFFirefoxIdTest();
			firefoxUserDisplayName = Config.getInstance().getUKFFirefoxDisplayNameTest();

		} else {
			webUserId = Config.getInstance().getUKFWebId();
			webUserDisplayName = Config.getInstance().getUKFWebDisplayName();
			chromeUserId = Config.getInstance().getUKFChromeId();
			chromeUserDisplayName = Config.getInstance().getUKFChromeDisplayName();
			firefoxUserId =  Config.getInstance().getUKFFirefoxId();
			firefoxUserDisplayName = Config.getInstance().getUKFFirefoxDisplayName();
		}
		...
		if( rc != 0 ) {
			...
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to UKF Web");
			System.exit(0);
		}	
		...
		if( rc != 0 ) {
			...
			System.out.println("exiting the programe...");
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to UKF Firefox");
			System.exit(0);
		}	
		...
		System.out.println("ChromUKFser Starting Conversation With Firefoxuser.");
...
//				webDriver.findElementByXPath("//div[@class='mainDiv']//div[2]//div[@class='scrollbox']/div[@class='table-view']//div[4]/div[@displayname='"+firefoxUserDisplayName+"']").click();
			...
			try {
				...
				Assert.assertFalse(true,"Error UKF WebUser Unable to Create COnversation with UKF Firefox User");
			}
			...
			if(webDriver.findElementByXPath("//textarea[@class='namegenTextLong']") != null) {
				...
				Message1 = Message1 +" : UKF - Hello from Webuser";
				...
				if (sentStaus.getAttribute("title").contains("Read") || sentStaus.getAttribute("title").contains("Sent") ) {
					...
					Assert.assertFalse(true,"Error UKF WebUser Unable to send message");
				}
			}
		} catch(Exception e) {
			System.out.println("Webuser Unable to create conversation with Firefoxuser.");
...
//		Thread.sleep(2000);
		...
		try {
			...
			System.out.println("ERROR- Conversation created by Webuser with Firefoxuser does not exists on Firefoxuser side Or Firefoxuser did not receive the message sent by ChromUKFser.");
			Assert.assertFalse(true,"ERROR - UKF Firefox user did not get the message");
		}
...
//				NetsfereActivity.attachImage_win_Robot();
		...
		try {
			...
			if(elems4.size() > 0) {
				...
				if ( el1 != null ) {					
					System.out.println("ChromUKFser Received Image");
				}
			}
		} catch ( NoSuchElementException e) {		
	...
	@Test(priority=6)
	public void UKF_Netsfere_HDCall_Check() throws InterruptedException, IOException , AWTException {	
		//******************************
		//  HD 1-1 Call 
		//******************************
		...
		// Webuser make a call to webUKFser2..
		String  webUserId = "";
		String  webUserDisplayName = "";
		String  chromeUserId = "";
		String  chromeUserDisplayName = "";
...
//		webDriver.findElementByXPath("//span[@class='icon ion-android-call' and @title='Call']").click();
		...
		try {
			...
			if ( foxDriver.findElementByXPath("//span[contains(text(),'Incoming call...')]") != null ) {				
				System.out.println("Firefoxuser Received the HD call from ChromUKFser");
			}
		...
		System.out.println("Login to Second ChromUKFser...");
...
//		}	
		...
		try {
			if ( NetsfereActivity.WebLogout(webDriver) ==0 ) {
				System.out.println("UKF Web User logout.");
				webDriver.quit();
			} else {
				System.out.println("UKF Web User logout failed.");
				webDriver.quit();
			}	
			if( foxDriver != null ) {
				if ( NetsfereActivity.WebLogout(foxDriver) ==0 ) {
					System.out.println("UKF Firefox User logout.");
					foxDriver.quit();
				} else {
					System.out.println("UKF Firefox User logout failed.");
					foxDriver.quit();
				}	
			...
			if( chromeDriver != null ) {
				if ( NetsfereActivity.WebLogout(chromeDriver) ==0 ) {
					System.out.println("UKF Chrome User logout.");
					chromeDriver.quit();
				} else {
					System.out.println("UKF Chrome User logout failed.");
					chromeDriver.quit();
				}	
			}
		...
		System.out.println("=================================================================");
		System.out.println("******** UKF REGION - Basic health checks Ended... ************"); 
