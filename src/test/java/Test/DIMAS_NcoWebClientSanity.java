...
public class DIMAS_NcoWebClientSanity  extends Base {
	...
	//	@Test(priority=4)
	public void Dimas_Netsfere_Cleanup() throws InterruptedException, IOException {
		...
		System.out.println("Dimas Cleanup has been completed: Success*******.");
	}

	@Test(priority=5)
	public void Dimas_Netsfere_Message_Call_Check() throws InterruptedException, IOException , AWTException {		

		System.out.println("Start Time : " + ( new SimpleDateFormat("yyyy:MM:dd-HH:mm:ss").format(new java.util.Date())));		

		NetsfereActivity.killAllBrowserSessions();	
		System.out.println("=========================================================================");
		System.out.println("******** DIMAS  Region - Basic health checks started... ************"); 
		...
		if( Config.getInstance().isDebug()) {
			webUserId = Config.getInstance().getDimasWebIdTest();
			webUserDisplayName = Config.getInstance().getDimasWebDisplayNameTest();
			chromeUserId = Config.getInstance().getDimasChromeIdTest();
			chromeUserDisplayName = Config.getInstance().getDimasChromeDisplayNameTest();
			firefoxUserId =  Config.getInstance().getDimasFirefoxIdTest();
			firefoxUserDisplayName = Config.getInstance().getDimasFirefoxDisplayNameTest();

		} else {
			webUserId = Config.getInstance().getDimasWebId();
			webUserDisplayName = Config.getInstance().getDimasWebDisplayName();
			chromeUserId = Config.getInstance().getDimasChromeId();
			chromeUserDisplayName = Config.getInstance().getDimasChromeDisplayName();
			firefoxUserId =  Config.getInstance().getDimasFirefoxId();
			firefoxUserDisplayName = Config.getInstance().getDimasFirefoxDisplayName();
		}
		...
		if( rc != 0 ) {
			...
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to Dimas Web");
			System.exit(0);
		}	
		...
		if( rc != 0 ) {
			...
			System.out.println("exiting the programe...");
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to Dimas Firefox");
			System.exit(0);
		}	
		...
		System.out.println("ChromDimasser Starting Conversation With Firefoxuser.");
		...
		try {
			...
			try {
				...
				Assert.assertFalse(true,"Error Dimas WebUser Unable to Create COnversation with Dimas Firefox User");
			}
			...
			if(webDriver.findElementByXPath("//textarea[@class='namegenTextLong']") != null) {
				...
				Message1 = Message1 +" : Dimas - Hello from Webuser";
				...
				if (sentStaus.getAttribute("title").contains("Read") || sentStaus.getAttribute("title").contains("Sent") ) {
					...
					Assert.assertFalse(true,"Error Dimas WebUser Unable to send message");
				}
			}
		} catch(Exception e) {
			System.out.println("Webuser Unable to create conversation with Firefoxuser.");
		...
		try {
			...
			System.out.println("ERROR- Conversation created by Webuser with Firefoxuser does not exists on Firefoxuser side Or Firefoxuser did not receive the message sent by ChromDimasser.");
			Assert.assertFalse(true,"ERROR - Dimas Firefox user did not get the message");
		}
		...
		try {
			...
			if(elems4.size() > 0) {
				...
				if ( el1 != null ) {					
					System.out.println("ChromDimasser Received Image");
				}
			}
		} catch ( NoSuchElementException e) {		
			System.out.println("ChromDimasser DID NOT recieve the IMAGE from ChromDimasser....  ");
		...
		// Webuser make a call to webDimasser2..
		...
		try {
			...
			if ( foxDriver.findElementByXPath("//span[contains(text(),'Incoming call...')]") != null ) {				
				System.out.println("Firefoxuser Received the HD call from ChromDimasser");
			}
		...
		System.out.println("Login to Second ChromDimasser...");
		...
		if( rc != 0 ) {
			...
			System.out.println("exiting the programe...");
			webDriver.close();
			Assert.assertFalse(true,"Unable to login to ChromDimasser");
			System.exit(0);
		}		
		...
		try {
			...
			if ( foxDriver.findElementByXPath("//div[contains(text(),'Incoming call...')]") != null ) {	
				System.out.println("Firefoxuser Received the group call from SecondChromDimasser");
			}
		...
		try {
			...
			if ( chromeDriver.findElementByXPath("//div[contains(text(),'Incoming call...')]") != null ) {	
				System.out.println("ChromDimasser Received the group call from Webuser");
			}

			if(chromeDriver.findElementByXPath("//button[@title='Accept']") != null) {
				chromeDriver.findElementByXPath("//button[@title='Accept']").click();				
				System.out.println("ChromDimasser Accepted the call");								
			}
		} catch ( NoSuchElementException e) {		
		...
		try {
			if ( NetsfereActivity.WebLogout(webDriver) ==0 ) {
				System.out.println("Dimas Web User logout.");
				webDriver.quit();
			} else {
				System.out.println("Dimas Web User logout failed.");
				webDriver.quit();
			}	
			if( foxDriver != null ) {
				if ( NetsfereActivity.WebLogout(foxDriver) ==0 ) {
					System.out.println("Dimas Firefox User logout.");
					foxDriver.quit();
				} else {
					System.out.println("Dimas Firefox User logout failed.");
					foxDriver.quit();
				}	
			...
			if( chromeDriver != null ) {
				if ( NetsfereActivity.WebLogout(chromeDriver) ==0 ) {
					System.out.println("Dimas Chrome User logout.");
					chromeDriver.quit();
				} else {
					System.out.println("Dimas Chrome User logout failed.");
					chromeDriver.quit();
				}	
			}
		...
		System.out.println("=========================================================================");
		System.out.println("******** DIMAS  Region - Basic health checks Ended... ************"); 
