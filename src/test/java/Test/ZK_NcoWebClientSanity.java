...
public class ZK_NcoWebClientSanity extends Base {
	...
	//	@Test(priority=4)
	public void ZK_Netsfere_Cleanup() throws InterruptedException, IOException {
		...
		NetsfereActivity.killAllBrowserSessions();
		System.out.println("Zollernalb  Cleanup has been completed: Success*******.");
	}

	@Test(priority=5)
	public void ZK_Netsfere_Message_Call_Check() throws InterruptedException, IOException , AWTException {		

		NetsfereActivity.killAllBrowserSessions();	

		System.out.println("=================================================================");
		System.out.println("******** ZOLLERNALB REGION - Basic health checks started... ************"); 
		...
		if( Config.getInstance().isDebug()) {
			webUserId = Config.getInstance().getZKWebIdTest();
			webUserDisplayName = Config.getInstance().getZKWebDisplayNameTest();
			chromeUserId = Config.getInstance().getZKChromeIdTest();
			chromeUserDisplayName = Config.getInstance().getZKChromeDisplayNameTest();
			firefoxUserId =  Config.getInstance().getZKFirefoxIdTest();
			firefoxUserDisplayName = Config.getInstance().getZKFirefoxDisplayNameTest();

		} else {
			webUserId = Config.getInstance().getZKWebId();
			webUserDisplayName = Config.getInstance().getZKWebDisplayName();
			chromeUserId = Config.getInstance().getZKChromeId();
			chromeUserDisplayName = Config.getInstance().getZKChromeDisplayName();
			firefoxUserId =  Config.getInstance().getZKFirefoxId();
			firefoxUserDisplayName = Config.getInstance().getZKFirefoxDisplayName();
		}
		...
		if( rc != 0 ) {
			...
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to ZK Web");
			System.exit(0);
		}	
		...
		if( rc != 0 ) {
			...
			System.out.println("exiting the programe...");
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to ZK Firefox");
			System.exit(0);
		}	
		...
		System.out.println("ChromZKser Starting Conversation With Firefoxuser.");
		...
		try {
			...
			try {
				...
				Assert.assertFalse(true,"Error ZK WebUser Unable to Create COnversation with ZK Firefox User");
			}
			...
			if(webDriver.findElementByXPath("//textarea[@class='namegenTextLong']") != null) {
				...
				Message1 = Message1 +" : ZK - Hello from Webuser";
				...
				if (sentStaus.getAttribute("title").contains("Read") || sentStaus.getAttribute("title").contains("Sent") ) {
					...
					Assert.assertFalse(true,"Error ZK WebUser Unable to send message");
				}
			}
		} catch(Exception e) {
			System.out.println("Webuser Unable to create conversation with Firefoxuser.");
		...
		try {
			...
			System.out.println("ERROR- Conversation created by Webuser with Firefoxuser does not exists on Firefoxuser side Or Firefoxuser did not receive the message sent by ChromZKser.");
			Assert.assertFalse(true,"ERROR - ZK Firefox user did not get the message");
		}
		...
		try {
			...
			if(elems4.size() > 0) {
				...
				if ( el1 != null ) {					
					System.out.println("ChromZKser Received Image");
				}
			}
		} catch ( NoSuchElementException e) {		
	...
	@Test(priority=6)
	public void ZK_Netsfere_HDCall_Check() throws InterruptedException, IOException , AWTException {	
		//******************************
		//  HD 1-1 Call 
		//******************************
		...
		// Webuser make a call to webZKser2..
		...
		try {
			...
			if ( foxDriver.findElementByXPath("//span[contains(text(),'Incoming call...')]") != null ) {				
				System.out.println("Firefoxuser Received the HD call from ChromZKser");
			}
		...
		System.out.println("Login to Second ChromZKser...");
		...
		try {
			if(webDriver != null ) { 
				if ( NetsfereActivity.WebLogout(webDriver) ==0 ) {
					System.out.println("ZK Web User logged out.");
					webDriver.quit();
				} else {
					System.out.println("ZK Web User logout failed.");
					webDriver.quit();
				}	
			}
			if( foxDriver != null ) {
				if ( NetsfereActivity.WebLogout(foxDriver) ==0 ) {
					System.out.println("ZK Firefox User logged out.");
					foxDriver.quit();
				} else {
					System.out.println("ZK Firefox User logout failed.");
					foxDriver.quit();
				}	
			}
			if(chromeDriver != null) {
				if ( NetsfereActivity.WebLogout(chromeDriver) ==0 ) {
					System.out.println("ZK Chrome User logged out.");
					chromeDriver.quit();
				} else {
					System.out.println("ZK Chrome User logout failed.");
					chromeDriver.quit();
				}	
			}
		...
		System.out.println("=================================================================");
		System.out.println("******** ZK REGION - Basic health checks Ended... ************"); 
