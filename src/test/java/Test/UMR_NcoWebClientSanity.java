...
public class UMR_NcoWebClientSanity  extends Base {
	...
	//	@Test(priority=4)
	public void UMR_Netsfere_Cleanup() throws InterruptedException, IOException {
		...
		NetsfereActivity.killAllBrowserSessions();
		System.out.println("UMR Cleanup has been completed: Success*******.");
	}

	@Test(priority=5)
	public void UMR_Netsfere_Message_Call_Check() throws InterruptedException, IOException , AWTException {		

		NetsfereActivity.killAllBrowserSessions();	

		System.out.println("=================================================================");
		System.out.println("******** UMR REGION - Basic health checks started... ************"); 
		...
		if( Config.getInstance().isDebug()) {
			webUserId = Config.getInstance().getUMRWebIdTest();
			webUserDisplayName = Config.getInstance().getUMRWebDisplayNameTest();
			chromeUserId = Config.getInstance().getUMRChromeIdTest();
			chromeUserDisplayName = Config.getInstance().getUMRChromeDisplayNameTest();
			firefoxUserId =  Config.getInstance().getUMRFirefoxIdTest();
			firefoxUserDisplayName = Config.getInstance().getUMRFirefoxDisplayNameTest();

		} else {
			webUserId = Config.getInstance().getUMRWebId();
			webUserDisplayName = Config.getInstance().getUMRWebDisplayName();
			chromeUserId = Config.getInstance().getUMRChromeId();
			chromeUserDisplayName = Config.getInstance().getUMRChromeDisplayName();
			firefoxUserId =  Config.getInstance().getUMRFirefoxId();
			firefoxUserDisplayName = Config.getInstance().getUMRFirefoxDisplayName();
		}
		...
		if( rc != 0 ) {
			...
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to UMR Web");
			System.exit(0);
		}	
		...
		if( rc != 0 ) {
			...
			System.out.println("exiting the programe...");
			webDriver.close();
			Assert.assertFalse(true,"Error While Logging in to UMR Firefox");
			System.exit(0);
		}	
		...
		System.out.println("Webuser Starting Conversation With Firefoxuser.");
...
Thread.sleep(1000);
			...
			try {
				...
				Assert.assertFalse(true,"Error UMR WebUser Unable to Create COnversation with UMR Firefox User");
			}
			...
			if(webDriver.findElementByXPath("//textarea[@class='namegenTextLong']") != null) {
				...
				Message1 = Message1 +" : UMR - Hello from Webuser";
				...
				if (sentStaus.getAttribute("title").contains("Read") || sentStaus.getAttribute("title").contains("Sent") ) {
					...
					Assert.assertFalse(true,"Error UMR WebUser Unable to send message");
				}
			}
		} catch(Exception e) {
			System.out.println("Webuser Unable to create conversation with Firefoxuser.");
...
//		Thread.sleep(2000);
		...
		try {
			...
			System.out.println("ERROR- Conversation created by Webuser with Firefoxuser does not exists on Firefoxuser side Or Firefoxuser did not receive the message sent by ChromUMRser.");
			Assert.assertFalse(true,"ERROR - UMR Firefox user did not get the message");
		}
...
//				NetsfereActivity.attachImage_win_Robot();
		...
		try {
			...
			if(elems4.size() > 0) {
				...
				if ( el1 != null ) {					
					System.out.println("ChromUMRser Received Image");
				}
			}
		} catch ( NoSuchElementException e) {		
			System.out.println("ChromUMRser DID NOT recieve the IMAGE from ChromUMRser....  ");
		...
		// Webuser make a call to webUMRser2..
	...
	webWait5.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[text()='Call']/preceding-sibling::button[./span[@class='icon ion-android-call']]")));
		...
		try {
			...
			if ( foxDriver.findElementByXPath("//span[contains(text(),'Incoming call...')]") != null ) {				
				System.out.println("Firefoxuser Received the HD call from ChromUMRser");
			}
		...
		System.out.println("Login to Second ChromUMRser...");
		...
		if( rc != 0 ) {
			...
			System.out.println("exiting the programe...");
			webDriver.close();
			Assert.assertFalse(true,"Unable to login to ChromUMRser");
			System.exit(0);
		}		
...
//			Thread.sleep(5000);
		...
		try {
			...
			if ( foxDriver.findElementByXPath("//div[contains(text(),'Incoming call...')]") != null ) {	
				System.out.println("Firefoxuser Received the group call from SecondChromUMRser");
			}
...
//		}	
		...
		try {
			if ( NetsfereActivity.WebLogout(webDriver) ==0 ) {
				System.out.println("UMR Web User logout.");
				webDriver.quit();
			} else {
				System.out.println("UMR Web User logout failed.");
				webDriver.quit();
			}	
			if( foxDriver != null ) {
				if ( NetsfereActivity.WebLogout(foxDriver) ==0 ) {
					System.out.println("UMR Firefox User logout.");
					foxDriver.quit();
				} else {
			...
			if( chromeDriver != null ) {
				if ( NetsfereActivity.WebLogout(chromeDriver) ==0 ) {
					System.out.println("UMR Chrome User logout.");
					chromeDriver.quit();
				} else {
					System.out.println("UMR Chrome User logout failed.");
					chromeDriver.quit();
				}	
			}
		...
		System.out.println("=================================================================");
		System.out.println("******** UMR REGION - Basic health checks Ended... ************"); 
